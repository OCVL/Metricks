...
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        resampleX = change/10
        # resampleX = numpy.around(resampleX, decimals=4)
        minSample = min(scaledDrpSizes)
        maxSample = max(scaledDrpSizes)
        interPDrpX = numpy.linspace(minSample, maxSample, round((maxSample-minSample)/resampleX[0])+1)  #not sure about this - works for my test dataset but unsure why needed to add the 1
        interPDrpX = numpy.around(interPDrpX, decimals=4)
        ...
        if (len(scaledDrpSizes) >= 2) and (len(densityPerAnnulus) >= 2) and (len(interPDrpX) >= 2):
            ...
            # splined = interpolate.interpolatedUnivariateSpline(scaledDrpSizes, densityPerAnnulus)

            splined = scipy.interpolate.InterpolatedUnivariateSpline(scaledDrpSizes, densityPerAnnulus)
            plt.plot(scaledDrpSizes, densityPerAnnulus, 'ro', ms=5)
            plt.plot(interPDrpX, splined(interPDrpX), 'g', lw=3, alpha=0.7)
            plt.show()
            splined = splined(interPDrpX)
